...
import { PagamentoService } from "./pagamentos/pagamento.service.js";

export const ContratoService = {
  async createContrato(data: CreateContratoDto) {
    await UserService.getUserById(data.usuarioId);
    await VeiculosService.getVeiculoById(data.veiculoId);
  ...
  async updateStatusPagamentosByContratoId(
    ...
    const updatePagamento = await PagamentoService.updateStatusPagamento(
      pagamentoId,
      status
    );
